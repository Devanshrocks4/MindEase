...
// ── Call the Netlify function with FULL conversation history ───────
async function callGeminiAPI(messages) {
  try {
    // Send full conversation history so AI has context of the whole session
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) throw new Error('RATE_LIMIT');
      if (response.status === 400 && errData.error === 'INVALID_KEY') throw new Error('INVALID_KEY');
      throw new Error('API_ERROR');
    }
...
export default function Chat({ userId }) {
  ...
  const processAIResponse = useCallback((reply, updatedMessages) => {
    // Check for crisis marker
    if (reply.includes('[CRISIS_DETECTED]')) {
      setCrisis(true);
    }

    // Check for specialist referral marker
    const specialistMatch = reply.match(/\[NEEDS_SPECIALIST:(psychologist|neurologist|psychiatrist)\]/i);
    let cleanReply = reply
      .replace(/\[NEEDS_SPECIALIST:[^\]]+\]/gi, '')
      .replace(/\[CRISIS_DETECTED\]/gi, '')
      .trim();
  ...
  const send = useCallback(async (override) => {
    ...
    try {
      // Build history for API — include mood context in first message
      const history = updated.map((m, i) => ({
        role: m.role === 'assistant' ? 'assistant' : 'user',
        content: (i === updated.length - 1 && n === 1 && mood)
          ? `[User's current mood: ${mood.label}] ${m.content}`
          : m.content,
      }));
      ...
      if (err.message === 'INVALID_KEY') {
        errMsg = '⚠️ There is an issue with the AI configuration. Please contact support.';
      } else if (err.message === 'RATE_LIMIT') {
        errMsg = "I'm a bit busy right now 😊 Please wait a moment and try again — I'm here for you.";
      } else {
        const fallbacks = [
          "I'm having a brief connection issue. Whatever you're feeling right now is valid. Please try again in a moment — I'm here for you. 💙",
          "Connection hiccup on my end! If you need immediate support, iCall (9152987821) is available Mon–Sat 8am–10pm. Try sending your message again.",
        ];
        errMsg = fallbacks[Math.floor(Math.random() * fallbacks.length)];
      }
      setMessages(p => [...p, { id: Date.now()+1, role: 'assistant', content: errMsg, ts: new Date() }]);
