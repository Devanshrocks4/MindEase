// ─────────────────────────────────────────────────────────────────
//  src/components/Dashboard.js  (PATCH — only the data-loading useEffect changes)
//
//  Why: your original query
//      query(collection(db,'assessments'),
//            where('userId','==',userId),
//            orderBy('date','desc'))
//  REQUIRES a Firestore composite index ("userId asc, date desc").
//  Before the index is built, that query throws "failed-precondition"
//  and the dashboard shows "No assessments yet" even when records exist.
//
//  Fix: use currentUser.uid (not the anon `userId`), AND fall back to
//  a no-orderBy query if the index isn't ready yet, sorting client-side.
//
//  Drop this entire file in to replace your existing Dashboard.js.
// ─────────────────────────────────────────────────────────────────
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (isFirebaseConfigured && db && uid) {
          // Try the ideal query first (needs composite index).
          let snap;
          try {
            const q = query(
            ...
            // failed-precondition = index missing. Fall back to client-side sort.
            console.warn('[Dashboard] composite index not ready, falling back:', idxErr.code);
            const q2 = query(collection(db, 'assessments'), where('userId', '==', uid));
          ...
          // Demo / not signed in — local storage only.
          const local = JSON.parse(localStorage.getItem('mindease_assessments') || '[]');
          data = uid
            ? local.filter((a) => a.userId === uid)
            : local; // show all local results if not signed in
        }
        ...
        if (err.code === 'permission-denied') {
          toast.error('🔒 Firestore rules blocked the read. Deploy firestore.rules.', { duration: 6000 });
        }
        // Always show whatever we have locally so the UI never goes blank.
        const local = JSON.parse(localStorage.getItem('mindease_assessments') || '[]');
    ...
    // Wait until auth state is known before querying.
